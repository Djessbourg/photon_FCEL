...
#include "Riostream.h"
	...
	//~ TFile *f = new TFile("ggdtest_b.root");
  //TFile *f = new TFile("ggdtest_c.root");
  TFile *f = new TFile("ggdtest_d.root");
  //TFile *f = new TFile("ggdtest_d.root");
	TTree *t2 = (TTree*)f->Get("t2");
	//
    TFile f1("/home/guillet/devel_jetphox-1.2.2/pawres/ggdtest_d.root");
	f1.ls();
    //~ TH1D *h1d = (TH1D*)f1.Get("hp41");
    TH1D *h1 = (TH1D*)f1.Get("hp21");
  t2->Print();
	Int_t iprov,ntrack;
  ...
  xbin[nbin] = xmax;
	...
	TH1F *hx2 = new TH1F("x2","essai",100,1.e-5,1.);
  //
  ...
  c1->cd(1);
  gPad->SetLogy();
	// on fixe le maximum et le minimum de l'echelle vertical
	//~ hx3->SetMaximum(50);
	//~ hx3->SetMinimum(0);
	// pour eviter le titre
  h1->SetTitle("");
   h1->SetMaximum(200000);
   //~ gr1->SetMaximum(8000000);
   h1->SetMinimum(5.);
	// les titres des axes
  h1->GetXaxis()->SetTitle("P_{t #gamma}");
  h1->GetYaxis()->SetTitle("d #sigma/d P_{t #gamma}");
	//
  h1->Draw("AXIS");
  h1->Draw("");
  //~ hpt->SetLineColor(2);
